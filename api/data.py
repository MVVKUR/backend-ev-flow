"""Data layer: load + normalise PLN / OCM / OSM into one in-memory DataFrame.

Loaded once at process start. ~3.5k rows, so plain pandas filtering is plenty fast.
Re-uses the same normalisation as the analysis notebook.
"""
...
def load() -> pd.DataFrame:
    """Build (once) and return the unified stations DataFrame."""
    global _DF
    if _DF is not None:
        return _DF
...
def reload() -> pd.DataFrame:
    """Force a re-read from disk (e.g. after refreshing the raw data)."""
    global _DF
    _DF = None
    return load()
